...
struct CommunityDetectionProgram : public GraphChiProgram<VertexDataType, EdgeDataType> {
    ...
    void update(graphchi_vertex<VertexDataType, EdgeDataType> &vertex, graphchi_context &gcontext) {
        ...
        vid_t newlabel = vertex.get_data();
		if (gcontext.iteration == 0) {
			...
			if(newlabel != maxlabel){
				...
				for(int i=0; i<vertex.num_edges(); i++) {
					...
					if(scheduler) gcontext.scheduler->add_task(vertex.edge(i)->vertex_id());                
				}
    ...
    void before_iteration(int iteration, graphchi_context &info) {
		converged = iteration > 0 ? true : false;
    }
	...
	/**
	 * interval repeats updates?
	 */
	bool repeat_updates(graphchi_context &ginfo){
		if(ginfo.iteration > 0)
			std::cout<<"repeat updates interval_converged="<<interval_converged<<"\t iteration="
				<<ginfo.iteration<<std::endl;

		return !interval_converged;	
		/*
		   if(0 == ginfo.iteration){
		   return false;	
		   }else{
		   std::cout<<"repeat updates interval_converged="<<interval_converged<<"\t iteration="
		   <<ginfo.iteration<<std::endl;
...
int main(int argc, const char ** argv) {
    ...
    metrics m("block-based-community-detection");
    
    /* Basic arguments for application */
    std::string filename = get_option_string("file");  // Base filename
    int niters           = get_option_int("niters", 100000); // Number of iterations (max)
    scheduler       = get_option_int("scheduler", false);//true;    // Always run with scheduler
