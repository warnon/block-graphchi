	...
	void update(graphchi_vertex<VertexDataType, EdgeDataType> &vertex, graphchi_context &gcontext) {
		...
		if (gcontext.iteration == 0){
			if(vertex.id() == vdata.ccid){
				...
				//source vertex in each CC
					...
					for(int i=0; i<vertex.num_outedges(); i++){
						...
						edata.neighbor_label(vertex.id(), vertex.edge(i)->vertex_id()) = 0;
				...
				for(int i=0; i<vertex.num_outedges(); i++){
					bidirectional_label edata = vertex.outedge(i)->get_data();
					edata.neighbor_label(vertex.id(), vertex.outedge(i)->vertex_id()) = Max_Level;
					vertex.outedge(i)->set_data(edata);
				}
			}	
		}else{
			...
			for(int i=0; i < vertex.num_inedges(); i++) {
				bidirectional_label edata = vertex.inedge(i)->get_data();
				vid_t nblabel = edata.my_label(vertex.id(), vertex.inedge(i)->vertex_id()); 
			...
			if(old_value > curmin ){
				...
				for(int i=0; i<vertex.num_outedges(); i++){
					bidirectional_label edata = vertex.outedge(i)->get_data();
					edata.neighbor_label(vertex.id(), vertex.outedge(i)->vertex_id()) = curmin;
					vertex.outedge(i)->set_data(edata);
...
vid_t getNewId(vid_t cid, vid_t level){
	...
	if(row_idx < top){
		level = level == Max_Level ? (bfs_level[row_idx].size()-1) : level;	
	}else{
		level = 0;	
	}
 ...
 //   diff_iter			=get_option_int("diff",0);
	...
	for(int i=0; i<(int)cc_size.size(); i++){
		if(i < top){
			...
			// store only top CC(account for 98% vertices)
			if(i == top){
				prefix_sum.push_back(tmp_sum);	
			}
	...
	*/	
    ...
    count_bfs_size<Vertexinfo>(filename, (vid_t)top, Max_Level, cc_to_index, bfs_level);
	...
	for(int i=0; i<(int)bfs_level.size(); i++){
		...
		//vertices with maxlevel is accumulated together in each CC
		if((i+1) < (int)bfs_level.size()){
			//assert(start == prefix_sum[i+1]);	
			if(start < prefix_sum[i+1]){	
				bfs_level[i].push_back(start);
			}
		}
		//sum += total;
	...
	// dv denotes vertices and edges in directed BFS
	vfout = fopen((filename+".dv").c_str(), "w+");	
	efout = fopen((filename+".de").c_str(), "w+");
	...
	fclose(vfout);
	fclose(efout);
	std::cout<<"directed BFS based partitioning has been remap"<<std::endl;
