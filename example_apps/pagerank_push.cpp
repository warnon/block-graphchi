...
struct PagerankProgramInmem : public GraphChiProgram<VertexDataType, EdgeDataType> {
    ...
    std::vector<std::atomic<EdgeDataType> > pr1;
	std::vector<std::atomic<EdgeDataType> > pr2;
	// two pointer curr is for 
	std::vector<std::atomic<EdgeDataType> >* curr;
	std::vector<std::atomic<EdgeDataType> >* next;
	
    //PagerankProgramInmem(int nvertices) :   pr(nvertices, RANDOMRESETPROB) {}
    PagerankProgramInmem(int nvertices) :   pr1(nvertices),pr2(nvertices) {}
...
int main(int argc, const char ** argv) {
    ...
    int niters              = get_option_int("niters", 40);
