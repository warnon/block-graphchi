 ...
 */
struct CutProgram: public GraphChiProgram<VertexDataType, EdgeDataType> {
		...
		if (gcontext.iteration == 0) {
			...
			for(int i=0; i<vertex.num_outedges(); i++){
				...
				/*
				   ...
				   */
				vid_t edge_id = vertex.outedge(i)->vertex_id();
				if(gcontext.interval_st > edge_id || gcontext.interval_en < edge_id){
					lock.lock();
					num_cut_edges++;
					lock.unlock();
				}
			...
			if(vertex.num_outedges() > 0){
				elock.lock();	
				total_edges += vertex.num_outedges();	
				elock.unlock();
			}
			/*
			}else{
...
int main(int argc, const char ** argv) {
    ...
    metrics m("blockupdate-CutEdges");
 ...
 //   diff_iter			=get_option_int("diff",0);
    ...
    metrics_report(m);
	std::cout<<"cut_analysis\t"<<filename<<"\tnum of cut edges="<<num_cut_edges<<"\t total edges="<<total_edges
				<<"\t cut_ration="<<(float)num_cut_edges*100/(float)total_edges<<"%"<<std::endl;
    return 0;
}
