...
#pragma omp parallel for schedule(dynamic, 1)
		...
		bool isConverge(std::vector<svertex_t>& vertices )
		{
		...
		/*
		void coloring(std::vector<svertex_t> &vertices){
			metrics_entry me = m.start_time();
				
			size_t nvertices = vertices.size();
			std::vector<vid_t> shuffle(nvertices);
			for(int k=0; k < (int)nvertices; k++) shuffle[k] = k;
			std::random_shuffle(shuffle.begin(), shuffle.end());

			std::vector<int> colors(nvertices, 0);
			size_t num_colors = 0;
			for(int i=0; i<(int)nvertices; i++){
				if(!(vertices[shuffle[i]].parallel_safe)){
					svertex_t vt = vertices[shuffle[i]];
					//vid_t vid = vt.id();
					std::set<int> nvcolors;	
					//ncolors.clear();
					for(int j=0; j < vt.num_edges(); j++){
						vid_t nvid = vt.edge(j)->vertex_id(); 		
						if(nvid < sub_interval_en && nvid >= sub_interval_st){
							nvcolors.insert(colors[nvid - sub_interval_st]);		
						}
					}
					std::set<int>::iterator iter;
					for_each(iter = nvcolors.begin(); iter != nvcolors.end(); iter++){
						if(colors[shuffle[i]] != *iter) 
							break;
						else
							colors[shuffle[i]]++;	
					}	
					num_colors = std::max(num_colors, colors[shuffle[i]]);
				}	
			}
				
			m.stop_time(me, "interval-coloring");	
		}	
		 virtual void exec_updates_threads(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram,
        ...
        #pragma omp section
            ...
            m.stop_time(me, "execute-updates_threads");
        }
		*/
		///////////////////////////////////////////////////
