...
template <typename LabelType>
int count_bfs_size(std::string basefilename, int ntop, int maxlevel, std::map<int, int>& ccid_to_idx, std::vector< std::vector<int> >& bfs_vector){    
		...
		for(int i=0; i < nt; i++) { 
			...
			if(lab.level == 0xffffffff){
				...
				if(i_idx == ntop){
					...
					//vertices with maxlevel is accumulated separately
					if(lab.level < maxlevel){
