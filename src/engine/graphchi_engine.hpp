...
#pragma omp parallel for schedule(dynamic, 1)
            for(int p=-1; p < nshards; p++)  {
                if (p==(-1)) {
                    ...
                    if (!memoryshard->loaded()) {
						...
						//std::cout<<"memory shard has been loaded!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<std::endl;
					}
		...
		// color scheduling 
		//vid_t max_colors = 0;
		vid_t compute_colors(std::vector<svertex_t> &vertices, std::vector<vid_t> &vcolors){
			size_t nvertices = vertices.size();
			//std::vector<vid_t> vcolors(nvertices, 0);	 		
			std::vector<vid_t> random_orders(nvertices); 
			for(int idx=0; idx < (int)nvertices; idx++){
				random_orders[idx] = (vid_t)idx;	
			}
			std::random_shuffle(random_orders.begin(), random_orders.end());		
			std::set<vid_t> nb_colors; 
			vid_t max_colors = 0;
			for(int i=0; i<(int)nvertices; i++){
				vid_t vid = random_orders[i];	
				//vid_t real_vid = vid + sub_interval_st;
				svertex_t v = vertices[vid];
				//graphchi_edge<EdgeDataType>
				nb_colors.clear();
				for(int j=0; j<v.num_edges(); j++){
					graphchi_edge<EdgeDataType>* edge = v.edge(j);
					vid_t nid = edge->vertex_id();
					if(nid < sub_interval_st || nid > sub_interval_en) continue;
					vid_t nid_offset = nid - sub_interval_st;
					nb_colors.insert(vcolors[nid_offset]);
				}
				// get smallest unused color
				vid_t my_color = 0;
				for(std::set<vid_t>::iterator it=nb_colors.begin(); it != nb_colors.end(); it++){
					if(my_color != *it)
						break;
					else
						my_color++;
				}
				max_colors = my_color > max_colors ? my_color : max_colors;	
				vcolors[vid] = my_color;	
			}	
			return max_colors;
		}
		
		bool check_colors(std::vector<svertex_t> &vertices, std::vector<vid_t> &vcolors){
			for(int i=0; i<vertices.size(); i++){
				...
				for(int j=0; j<v.num_edges(); j++){
					...
					if(nid < sub_interval_st || nid > sub_interval_en) continue;
					vid_t nid_offset = nid - sub_interval_st;
					assert(vcolors[(int)nid_offset] != vcolors[i]);	
				}
			}	
			return true;
		}
		// execute updates using coloring scheduling	
        virtual void exec_updates_coloring(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram,
            ...
            */ 
            ...
            //m.start_time("coloring construct");
			int colors = compute_colors(vertices, vcolors);
			//check if compute_colors works correctly
			//check_colors(vertices, vcolors);	
			std::vector< std::vector<vid_t> > color_array(colors); 	
			for(int i=0; i<nvertices; i++){
				int color = (int) vcolors[i];		
				if(color >= (int)color_array.size())	
					color_array.resize(color+1);	
				color_array[color].push_back((vid_t)i);	
			}
            m.stop_time(me1, "coloring-construct");
			printf("colors=%d\n", colors);
			metrics_entry me = m.start_time();
            //metrics_entry me = m.start_time();
			do {
				...
				userprogram.before_exec_interval(sub_interval_st, sub_interval_en, chicontext);
				//////////////////
				for(int i=0; i<(int)color_array.size(); i++){	
					...
					#pragma omp parallel for
							for(int idx=0; idx < (int)color_array[i].size(); idx++) {
								// vid_t vid = sub_interval_st + (randomization ? random_order[idx] : idx);
								vid_t vid = sub_interval_st + color_array[i][idx];
								assert(!(vid < sub_interval_st || vid > sub_interval_en));
                        ...
                        for(int idx=0; idx <= (int)sub_interval_len; idx++) {
                                ...
                                if (exec_threads == 1 || v.parallel_safe) {
                                    if (!disable_vertexdata_storage)
                                        v.dataptr = vertex_data_handler->vertex_data_ptr(vid);
                                    //if (v.scheduled) 
									//if(scheduler->is_scheduled(vid))
									if(chicontext.scheduler->is_scheduled(vid))
                                        userprogram.update(v, chicontext);
                                }
                            }
                        }
        #pragma omp section
                        {
                            if (exec_threads > 1 && enable_deterministic_parallelism) {
                                ...
                                for(int idx=0; idx <= (int)sub_interval_len; idx++) {
                                    ...
                                    //if (!v.parallel_safe && scheduler->is_scheduled(vid)) {
                                    if (!v.parallel_safe && chicontext.scheduler->is_scheduled(vid)) {
                                        if (!disable_vertexdata_storage)
                                            v.dataptr = vertex_data_handler->vertex_data_ptr(vid);
                                        userprogram.update(v, chicontext);
                                        nonsafe_count++;
				...
				//any_scheduled = is_any_vertex_scheduled(sub_interval_st, sub_interval_en);
				//inconsistent for interval_converged and scheduling queue.	
				/*
				if(!userprogram.repeat_updates(chicontext) && chicontext.iteration != 0){
					assert(!any_scheduled);	
				}	
				*/	
				//count++;
            //} while (chicontext.iteration != 0 && any_scheduled);
			//std::cout<<"interval="<<chicontext.exec_interval<<"\tcount="<<count<<std::endl;
            } while (userprogram.repeat_updates(chicontext));
        ...
        virtual void init_vertices(std::vector<svertex_t> &vertices, graphchi_edge<EdgeDataType> * &edata) {
            ...
            for(int i=0; i < (int)nvertices; i++) {
                ...
                if (scheduler != NULL) {
					...
					//////////////////////////////////////////////
					//modified by mzj, read in all vertices in the interval.
                    bool is_sched = true;
                    if (is_sched) {
                        vertices[i].scheduled =  true;
                        nupdates++;
    ...
    public:
        ...
        bool is_any_vertex_scheduled(vid_t st, vid_t en) {
            ...
            for(vid_t v=st; v<=en; v++) {
                if (scheduler->is_scheduled(v)) {
					//std::cout<<"st="<<st<<"\ten="<<en<<"\tvid="<<v<<std::endl;
                    return true;
                }
            }
            return false;
        ...
        void run(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, int _niters) {
			...
			/* last_iteration of chicontext should be set to -1 in each run
				 important bugfix by mzj 2016/3/12
			*/
			chicontext.last_iteration = -1;
            if (scheduler == NULL) {
            ...
            for(iter=0; iter < niters; iter++) {
                ...
                for(int interval_idx=0; interval_idx < nshards; ++interval_idx) {
					...
					/////////////////
					// for repeat update use
                   	chicontext.exec_interval = exec_interval;
					/////////////////
                    ...
                    while (sub_interval_st <= interval_en) {
                        ...
                        ////////modified by mzj
						chicontext.set_interval_range(sub_interval_st, sub_interval_en);	
						///////
                        ...
                        /* Execute updates */
                        if (!is_inmemory_mode()) {
							if(1 == get_option_int("color", 0))
								exec_updates_coloring(userprogram, vertices);		
							else
                            	exec_updates(userprogram, vertices);
			...
			//add by mzj, free allocated object scheduler
			if(chicontext.scheduler != NULL){
				delete chicontext.scheduler;
				chicontext.scheduler = NULL;		
				scheduler = NULL;
			}
        }
