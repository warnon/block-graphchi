...
#define VARIABLE_IS_NOT_USED
    ...
    template<typename EdgeDataType_, typename VertexDataType_>
    static VARIABLE_IS_NOT_USED void delete_all_auxfiles(std::string base_filename, int nshards){
		delete_shards<EdgeDataType_>(base_filename, nshards);
		/* vertex data file */
        //std::string vdata_filename = filename_vertex_data<VertexDataType_>(base_filename);
		std::string deltalog_filename = base_filename+".deltalog";
		/*	
        if (file_exists(vdata_filename)) {
            int err = remove(vdata_filename.c_str());
            if (err != 0) logstream(LOG_ERROR) << "Error removing file " << vdata_filename
                << ", " << strerror(errno) << std::endl;
        }
		*/	
		if (file_exists(deltalog_filename)) {
            int err = remove(deltalog_filename.c_str());
            if (err != 0) logstream(LOG_ERROR) << "Error removing file " << deltalog_filename
                << ", " << strerror(errno) << std::endl;
        }
        std::string numv_filename = base_filename + ".numvertices";
        if (file_exists(numv_filename)) {
 ...
 template<typename VertexDataType_>
    static VARIABLE_IS_NOT_USED void delete_vertexdata_file(std::string vdatafilename){
		//delete_shards<EdgeDataType_>(base_filename, nshards);
		/* vertex data file */
        std::string vdata_filename = filename_vertex_data<VertexDataType_>(vdatafilename);
        if (file_exists(vdata_filename)) {
            int err = remove(vdata_filename.c_str());
            if (err != 0) logstream(LOG_ERROR) << "Error removing file " << vdata_filename
                << ", " << strerror(errno) << std::endl;
 ...
 static VARIABLE_IS_NOT_USED void delete_component_file(std::string basefilename){
	  
		std::string outname = basefilename + ".components";
		//if(fileexit)	
		if(file_exists(outname)){
			if(0 != remove(outname.c_str())){
				logstream(LOG_ERROR)<<"Error removing file "<<outname<<std::endl;	
			}else
				std::cout<<" file "<<outname<<" has been removed "<<std::endl;
		}
	}
    /**
